...
//     $sql = "INSERT INTO tbl_payment(disName, disNo, invoiceNo, sumTotal,sumDiscount, xtotal,cash, dt, received,subtotal,salestax,cusType,staffid,dtNow,sumQTY,shiftCode) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
//     $q = $db->prepare($sql);
//     $q->execute(array($disName, $disNo, $invoiceNo, $subtotal, $sumDiscount, $xtotal, $cash, $dt, '1', $subtotal, $salestax, $disCat, $staffid, $dtNow,$_SESSION['sumQTY'],$_SESSION['shiftCode']));


//     $q = $db->prepare("UPDATE tbl_purchases set xfinished='1' WHERE invoiceNo='$invoiceNo'");
//     $q->execute(array());
// //New Customer -> Gen new invoice
// // ************ G E N E R A T E     N E W     I N V O I C E    N U M B E R  ****************
